...
public class DKnob extends JComponent
{
    ...
    //Added by David Gamez
    private String minRange = "0";
    private String maxRange = "1";
    private ArrayList<KnobDoubleClickListener> doubleClickListenerList = new ArrayList<KnobDoubleClickListener>();
        ...
        addMouseListener(new MouseAdapter() {
                ...
                public void mouseClicked(MouseEvent me) {
                    if(me.getClickCount() == 2){
                        fireKnobDoubleClickEvent(me.getX(), me.getY());
                    }
                    else {hitArc.setAngleExtent(-(LENGTH + 20));
                        if (hitArc.contains(me.getX(), me.getY())) {
                            hitArc.setAngleExtent(MULTIP * (ang - START_ANG) -
                                                  10);
                            if (hitArc.contains(me.getX(), me.getY())) {
                                decValue();
                            } else incValue();
  ...
  //  }
    ...
    public void setRange(int min, int max){
        minRange = String.valueOf(min);
        maxRange = String.valueOf(max);
        this.repaint();
    }
    ...
    public void addDoubleClickListener(KnobDoubleClickListener dcl){
        doubleClickListenerList.add(dcl);
    }

    public void removeDoubleClickListener(KnobDoubleClickListener dcl){
        doubleClickListenerList.add(dcl);
    }
    ...
    private void fireKnobDoubleClickEvent(int xPos, int yPos){
        for(KnobDoubleClickListener tempDcl : doubleClickListenerList)
            tempDcl.knobDoubleClicked(xPos, yPos);
    }
    ...
    public void paint(Graphics g) {
        ...
        //"0" replaced here by minRange - David Gamez
        g.drawString(minRange, 5, height-5);
        g.drawString(maxRange, size+5, height-5);
        ...
        g.setColor(Color.white);
        if(controllerText.length() == 1)
            g.drawString(controllerText, middle -2, middle + 13);
        else if(controllerText.length() == 2)
             g.drawString(controllerText, middle -5, middle + 13);
        else
             g.drawString(controllerText, middle -8, middle + 13);
    }
}
