...
public class CopyPanel extends AgentPropertiesPanel implements ConnectionListener, Constants, KnobDoubleClickListener, ControllerEventListener, ChangeListener{
	...
	public void loadAgentProperties() throws Exception {
		//Load up the names of the from and to tracks
		loadTrackNames();
	...
	private void loadTrackNames(){
		...
		for(String trkID : tmpTrkMap.keySet()){
			...
			//Store index if it is the from or to track
			if( trkID.equals( getCopyAgent().getFromTrackID() ) )
				fromSelIndx = cntr;
			if( trkID.equals( getCopyAgent().getToTrackID() ) )
				toSelIndx = cntr;
		...
		//Set selected index
		if(fromSelIndx >=0)
			fromCombo.setSelectedIndex(fromSelIndx);
		if(toSelIndx >=0)
			toCombo.setSelectedIndex(toSelIndx);
	}
	
	
	/** Sets the from and to connection of the agent */
	private void setCopyConnection(){
		//Do nothing if agent is not connected
		if(fromCombo.getItemCount() == 0)
			return;
		
		//Double check combos match list of ids
