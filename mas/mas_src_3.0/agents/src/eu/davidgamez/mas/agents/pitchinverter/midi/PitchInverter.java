...
public class PitchInverter extends Agent implements Constants {
	    	...
	    	for(Long key : tmpMsgMap.keySet()){
	    		...
	    		for(ShortMessage msg : tmpMsgMap.get(key)){
	    			...
	    			if(msg.getCommand() == ShortMessage.NOTE_ON || msg.getCommand() == ShortMessage.NOTE_OFF){
	    				...
	    				if(oldPitch > inversionPitch)
	    					newPitch = inversionPitch - (oldPitch - inversionPitch);
	    				
	    				else
	    					newPitch = inversionPitch + (inversionPitch - oldPitch);
	    				
	    				//Make sure pitch does not go out of range
	    				if(newPitch < 0)
	    					newPitch = 0;
	    				if(newPitch > 127)
	    					newPitch = 127;
	...
	public void setInversionPitch(int inversionPitch) throws MASAgentException {
		if(inversionPitch < 0 || inversionPitch > 127)
			throw new MASAgentException("Inverter Agent: Inversion pitch out of range: " + inversionPitch);
