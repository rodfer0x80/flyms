...
public class CopyAgent extends Agent implements Constants  {
	...
	public void addTrack(Track mTrack){
		...
		//Store track as from track or to track if it has the correct id
	...
	public void setCopyConnection(String tmpFromTrackID, String tmpToTrackID) throws MASAgentException {
		...
		if(!trackMap.containsKey(tmpFromTrackID))
			throw new MASAgentException("CopyAgent: From track with ID " + tmpFromTrackID + " does not exist.");	
		if(!trackMap.containsKey(tmpToTrackID))
			throw new MASAgentException("CopyAgent: To track with ID " + tmpToTrackID + " does not exist.");	
		
		
		//Set the from and to tracks
	...
	public void setDelay_beats(int delay_beats) throws MASAgentException {
		if(delay_beats < 0 || delay_beats > MAX_DELAY_BEATS)
			throw new MASAgentException("CopyAgent: Delay out of range: " + delay_beats);
	...
	protected boolean updateTracks(long bufferStart_ppq, long bufferEnd_ppq) throws InvalidMidiDataException {
		//Do nothing if from and to have not been set
		if(fromTrack == null || toTrack == null)
			return true;
		
		//Copy the buffer from the from track into the delay buffer
		copyBuffer();
		
		//Output notes with dealy
